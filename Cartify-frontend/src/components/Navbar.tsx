...
export const Navbar: React.FC<NavbarProps> = ({
  ...
  const handleSearchToggle = () => {
    if (isSearchOpen) { setSearchValue(''); onSearch(''); }
  ...
  const handleLinkClick = (e: React.MouseEvent, label: string) => {
    ...
    if (label === 'Shop' || label === 'Shop All') {
      ...
      navigate('/');
      setTimeout(() => document.querySelector('footer')?.scrollIntoView({ behavior: 'smooth' }), 100);
    } else {
  ...
  const useDarkText = isLightForcedPage || isPastProductsSection;

  const getTextColor = () => {
    if (isLightForcedPage) return isDark ? 'text-white' : 'text-gray-900';
    return isScrolled ? 'text-gray-900' : 'text-white';
  };

  const getLogoColor = () => {
    if (isLightForcedPage || isPastProductsSection) return isDark ? 'text-white' : 'text-gray-900';
    return 'text-white';
  };
  ...
  const getNavBg = () => {
    if (isLightForcedPage) return isDark ? 'bg-brand-950 shadow-md' : 'bg-white shadow-md';
    if (isScrolled) return isDark ? 'bg-brand-950 shadow-md' : 'bg-white shadow-md';
  ...
  const getToggleButtonColor = () => {
    if (isMobileMenuOpen || useDarkText) return isDark ? 'text-white' : 'text-gray-900';
    return 'text-white';
  };
  ...
  const getUserGradient = () => {
    ...
    const hash = user.email.split('').reduce((acc, c) => acc + c.charCodeAt(0), 0);
    const gradients = [
  ...
  const LetteredAvatar = ({ initials, gradient }: { initials: string; gradient: string }) => (
    <div className="relative group">
      <div
        className={cn(
          'w-8 h-8 sm:w-9 sm:h-9 rounded-full bg-gradient-to-br shadow-lg ring-2 ring-white/50',
  ...
  // ─── Mobile menu classes — now use isDark prop instead of Tailwind dark: variants ───

  const mobileBg     = isDark ? 'bg-brand-950'  : 'bg-white';
  const mobileBorder = isDark ? 'border-brand-800' : 'border-gray-200';
  const mobileText   = isDark ? 'text-white'    : 'text-gray-900';
  const mobileHover  = isDark
    ? 'hover:text-brand-400 hover:bg-brand-800'
    : 'hover:text-brand-600 hover:bg-gray-50';
  const mobileUserBg  = isDark ? 'bg-brand-800'  : 'bg-gray-50';
  const mobileCloseBtn = isDark
    ? 'text-gray-300 hover:bg-brand-800'
    : 'text-gray-700 hover:bg-gray-100';
  const mobileSignedInLabel = isDark ? 'text-emerald-400' : 'text-emerald-600';
  const mobileLogout = isDark
  ...
  return (
    <nav
      ...
      className={cn(
        ...
        'overflow-visible shrink-0 transition-colors duration-300',
        getNavBg()
      )}
    >
      <div className="max-w-7xl mx-auto w-full flex items-center justify-between h-full">
        ...
        <div className="flex-1 lg:flex-none flex justify-center lg:justify-start">
          <Link to="/" className="group relative flex items-center space-x-3 transition-all duration-300">
            <div className="relative">
              <div className="relative w-10 h-10 sm:w-12 sm:h-12 rounded-2xl bg-gradient-to-br from-brand-500 via-brand-600 to-brand-700 flex items-center justify-center shadow-2xl transform transition-all duration-500 group-hover:scale-110 group-hover:rotate-3 overflow-hidden">
                ...
                <span className="text-white font-black text-2xl sm:text-3xl tracking-tighter relative z-10 drop-shadow-lg">C</span>
            ...
            <div className="flex flex-col">
              <span className={cn('text-xl sm:text-2xl font-black tracking-tighter transition-all duration-300 relative group-hover:tracking-tight', logoColor)}>
                CARTIFY
        ...
        <div className="hidden lg:flex items-center space-x-6 xl:space-x-8 text-xs xl:text-sm font-medium uppercase tracking-widest">
          {desktopLinks.map(({ label, href, isAnchor }) =>
            isAnchor ? (
              <a
                key={label}
                href={href}
        ...
        <div className="flex items-center space-x-1 sm:space-x-2 md:space-x-4">
          ...
          >
            ...
            <input
              ...
              className={cn('bg-transparent border-none outline-none text-xs w-full cursor-text', searchTextColor, searchPlaceholderColor)}
            />
            {searchValue && (
              <button
                type="button"
        ...
        {isMobileMenuOpen && (
          <>
            ...
            <motion.div
              ...
              className={cn('fixed inset-y-0 left-0 w-[280px] sm:w-[320px] z-[101] shadow-2xl flex flex-col min-h-screen', mobileBg)}
            >
              {/* Header */}
              <div className={cn('flex items-center justify-between p-6 border-b', mobileBorder, mobileBg)}>
                <Link to="/" onClick={() => setIsMobileMenuOpen(false)} className="flex items-center space-x-2">
                  ...
                  <span className={cn('text-xl font-serif font-bold tracking-tighter', mobileText)}>CARTIFY</span>
                </Link>
                <button
                  onClick={() => setIsMobileMenuOpen(false)}
              ...
              <div className={cn('flex-1 overflow-y-auto py-6 px-6', mobileBg)}>
                <div className="flex flex-col space-y-1">
                  {mobileLinks.map(({ label, href, isAnchor }) =>
                    isAnchor ? (
                      <a
                        key={label}
                        href={href}
                    ...
                    <>
                      <div className={cn('flex items-center space-x-3 px-4 py-2 rounded-xl', mobileUserBg)}>
                        ...
                        <div className="flex-1">
                          <p className={cn('text-[10px] font-bold uppercase tracking-widest', mobileSignedInLabel)}>
                            Signed in as
                          </p>
                      ...
                      <button
                        ...
                        className={cn('flex items-center space-x-3 py-3 px-4 -mx-4 rounded-lg transition-colors text-base font-medium', mobileLogout)}
                      >
                        <span>Logout</span>
                      </button>
                    </>
