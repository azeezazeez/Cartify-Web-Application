...
export const Navbar: React.FC<NavbarProps> = ({
  ...
  const useDarkText =
    ...
    isScrolled; // Added isScrolled here

  const getTextColor = () => {
    if (useDarkText) {
      return 'text-gray-900'; // Black color
    }
    return 'text-white';
  };
    ...
    >
      <div className="max-w-7xl mx-auto w-full flex items-center justify-between h-full">
        ...
        <div className="flex items-center space-x-4">
          <button className={cn('p-2 rounded-full transition-colors', buttonBgHover, textColor)}>
          ...
          {user ? (
            <div className="relative" ref={userMenuRef}>
              ...
              {isUserMenuOpen && (
                <div className="absolute right-0 mt-2 w-48 bg-white rounded-xl shadow-xl border border-gray-100 py-2 py-2">
      ...
      {/* Mobile Menu Placeholder - Simplified for brevity in this response */}
      <AnimatePresence>
        {isMobileMenuOpen && (
          <motion.div
            initial={{ x: '-100%' }}
