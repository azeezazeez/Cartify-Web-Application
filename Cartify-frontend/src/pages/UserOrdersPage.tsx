...
export const UserOrdersPage = ({ isDark = false }: UserOrdersPageProps) => {
    ...
    // ─── Theme tokens — always white/light regardless of isDark ──────────────
    ...
    const inputBg      = 'bg-gray-50 border-gray-200 focus:border-gray-900';
    const inputText    = 'text-gray-900';
    const selectBg     = 'bg-gray-50 border-gray-200 focus:border-gray-900 text-gray-900';
    const sectionBg    = 'bg-gray-50';
    const iconBg       = 'bg-gray-100';
    const iconText     = 'text-gray-500';
    const emptyIcon    = 'text-gray-300';
    const btnPrimary   = 'bg-gray-900 text-white hover:bg-gray-800';
    const btnOutline   = 'border border-gray-200 text-gray-900 hover:bg-gray-50';
    const viewDetailsTxt = 'text-gray-500 hover:text-gray-900';
                    ...
                    <div className="space-y-4">
                        {filteredOrders.map((order, index) => (
                            ...
                            >
                                ...
                                {/* Footer */}
                                <div className={`px-6 py-4 flex justify-end border-t ${cardBorder} ${sectionBg}`}>
                                    <button
...
const OrderDetailsModal = ({
    order, onClose, formatDate, getStatusColor, getStatusIcon, formatCurrency,
    ...
    return (
        <>
            ...
            <motion.div
                ...
                className="fixed inset-4 md:inset-auto md:top-1/2 md:left-1/2 md:-translate-x-1/2 md:-translate-y-1/2 md:w-full md:max-w-lg z-[210] rounded-2xl overflow-hidden shadow-2xl flex flex-col max-h-[90vh] bg-white"
            >
                {/* Header */}
                <div className="p-6 border-b border-gray-100 flex items-center justify-between flex-shrink-0">
                    <div>
                        <h2 className="text-xl font-serif font-bold text-gray-900">Order Details</h2>
                        <p className="font-mono text-sm mt-0.5 text-gray-400">{order.orderId}</p>
                    </div>
                    <button
                        onClick={onClose}
                ...
                <div className="p-6 overflow-y-auto flex-1">
                    ...
                    <h3 className="text-sm font-semibold uppercase tracking-wide mb-3 text-gray-400">Items Ordered</h3>
                    <div className="space-y-3 mb-6">
                        {(order.items || []).map((item, idx) => (
                            <div key={idx} className="flex items-center gap-3 p-3 rounded-xl bg-gray-50">
                                ...
                                <div className="flex-1 min-w-0">
                                    <p className="text-sm font-medium truncate text-gray-900">{item.productName}</p>
                                    <p className="text-xs mt-0.5 text-gray-400">{formatCurrency(item.price)} × {item.quantity}</p>
                                </div>
                                <p className="text-sm font-semibold flex-shrink-0 text-gray-900">{formatCurrency(item.subtotal)}</p>
                            </div>
                        ))}
                    </div>
